...
from YOLO_Video import video_detection, detected_nutrition_info
...
def generate_frames(path_x=''):
    ...
    for detection_, nutrition_info in yolo_output:
        ref, buffer = cv2.imencode('.jpg', detection_)

        frame = buffer.tobytes()
        session['nutrition_info'] = nutrition_info  # Save detected nutrition info in session
        yield (b'--frame\r\n'
...
def nutrition_info():
    return jsonify(detected_nutrition_info)
